...
from ContinueException import Continue


def initialize_blocks(dims: int, points: list, threshold: float):
...
def search(points: list, threshold=2 / 3, processed_blocks=()):
    ...
    dims = len(points[0])
    blocks, blocks_ranges, blocks_hashes = initialize_blocks(dims, points, threshold)
    ...
    for point in points:
        for block_id, block in enumerate(blocks):
            try:
                ...
                # Force a continue in the outer-loop
                continue
    ...
    for block_id, block in enumerate(blocks):
        ...
        # More than 2 points, keep subdividing
            ...
            local_length, local_points = search(block, threshold, processed_blocks)
    ...
    if proximity_length is None:
        ...
        proximity_length, proximity_points = brute_force(points)
